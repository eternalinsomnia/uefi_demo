...
VOID draw_border() {
	...
	for(UINTN i = 0; i < handle_count; ++i) {
		...
		graphics_output[i]->Blt(graphics_output[i], border, EfiBltBufferToVideo, 0, 0, 
...
VOID rotate() {
	...
	UINTN temp_y;
	for(UINTN i = 0; i < 4; ++i) {
		temp_x = point[i].y - center.y;
		temp_y = point[i].x - center.x;
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
	...
	gBS->CreateEvent(EVT_TIMER, 0, NULL, NULL, &events[1]);
	gBS->SetTimer(events[1], TimerPeriodic, 5000*1000);
	...
	do {
		...
		gBS->WaitForEvent(2, events, &index_);
		if(index_ == 0)
